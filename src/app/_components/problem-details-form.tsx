...
export function ProblemDetailsForm({
  ...
  if (errors && Object.keys(errors).length) {
    ...
    const errorDescription = getErrorDescription(contactLinks, errorParam, descParam, formJson.problem, formJson.subProblem);
    ...
    >
      <fieldset className="nhsuk-fieldset">
        ...
        <div className="nhsuk-radios" id="common-problems" onChange={(e: any) => {
            if (e.target.name === 'problem') setShowOtherIssues(e.target.value === 'dupe_account');
          ...
          <div
            ...
            id="issues-list"
          >
            <RadioItem inputId="FirstLinePasswordReset" name="subProblem" value="first_line_password_reset">
            I cannot remember the password I used to set up my account
          </RadioItem>
            <RadioItem inputId="FirstLineChangeMobile" name="subProblem" value="first_line_change_mobile">
            I cannot access my account as I have changed my mobile number
          </RadioItem>
            <RadioItem inputId="SecondLineEmailHint" name="subProblem" value="second_line_email_hint">
            I cannot remember the email address I used to set up my account
          </RadioItem>
            <RadioItem inputId="SecondLineManualReset" name="subProblem" value="second_line_manual_reset">
...
function getProblemText(ref: RefObject<HTMLDivElement>, value?: string) {
  ...
  let errorDescription = errorDescriptions.find(i => i.problemValue === problemSelected) || null;
  if (errorDescription) {
...
function sendToApi(
  { name, email, contactEmail, nhsNumberLastDigits, phoneNumber, client, visit, 'message-detail': messageDetail }: ContactFormValues,
